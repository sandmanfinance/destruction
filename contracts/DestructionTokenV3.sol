...
// DestructionToken
contract DestructionTokenV3 is ERC20, Ownable {
    ...
    modifier antiBot(address _sender, address _recipient, uint256 _amount) { 
        //check blacklist
        require(!blacklistCheck(_sender), "ERR1");
        require(!blacklistCheck(_recipient), "ERR2");
    ...
    function blacklistCheck(address botAddress) public view returns(bool){
        if(_blacklist[botAddress] > 0)
            return _blacklist[botAddress] > block.timestamp;
        else 
            return false;
    }
    
    ///@dev check if the address is in the blacklist or not
    function blacklistCheckExpirationTime(address botAddress) public view returns(uint256){
